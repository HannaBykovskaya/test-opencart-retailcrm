...
class ControllerSaleOrder extends Controller {
	...
	public function getForm() {
		...
		// Загружаем модель для работы с заказами
		$this->load->model('sale/order');
		...
		if ($order_info) {
			// Передаем информацию о заказе в массив данных
			$data['delivery_date'] = $order_info['delivery_date'];
			$data['track_number'] = $order_info['track_number'];
			$data['desired_delivery_datetime'] = $order_info['desired_delivery_datetime'];
			$data['need_call'] = $order_info['need_call'];
		} else {
			$data['delivery_date'] = '';
			$data['track_number'] = '';
			$data['desired_delivery_datetime'] = '';
			$data['need_call'] = 0;
		}
		...
		if (!empty($order_info)) {
			...
			$data['desired_delivery_datetime'] = null;
			$data['need_call'] = 0;
