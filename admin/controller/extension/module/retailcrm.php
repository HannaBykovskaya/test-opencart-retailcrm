...
class ControllerExtensionModuleRetailcrm extends Controller
{
	...
	public function sync() {
        ...
        $this->model_extension_module_retailcrm->syncCustomersFromCRM();
        $this->response->setOutput('Synchronization completed successfully!');
    }
	...
	public function exportOrder() {
		...
		if (!isset($data['fromApi'])) {
			...
			$desired_delivery_datetime = isset($data['desired_delivery_datetime']) ? $data['desired_delivery_datetime'] : null;
			$need_call = isset($data['need_call']) ? (int)$data['need_call'] : 0;
