...
class ModelExtensionRetailcrmHistory extends Model {
	...
	protected function updateOrders($orders, $retailcrmApiClient) {
		foreach ($orders as $order) {
			...
			$delivery_date = !empty($order['customFields']['delivery_date']) ? $order['customFields']['delivery_date'] : null;
			$track_number = !empty($order['customFields']['track_number']) ? $order['customFields']['track_number'] : null;

			$this->db->query(
				"UPDATE " . DB_PREFIX . "order 
					SET delivery_date = '" . ($delivery_date !== null ? $this->db->escape($delivery_date) : '') . "', 
						track_number = '" . ($track_number !== null ? $this->db->escape($track_number) : '') . "'
					WHERE order_id = " . (int)$order['externalId']
			);
		}
	}
	...
	protected function updateCustomers($customers) {
		foreach ($customers as $customer) {
			...
			$children = isset($customer['customFields']['children']) ? $customer['customFields']['children'] : null;
			$operator_comment = isset($customer['customFields']['operator_comment']) ? $customer['customFields']['operator_comment'] : null;
	
			$custom_field = isset($customer['custom_field']) ? stripslashes($customer['custom_field']) : '';
	
			$this->db->query(
				"UPDATE " . DB_PREFIX . "customer 
				 SET children = '" . ($children !== null ? $this->db->escape($children) : '') . "', 
					 operator_comment = '" . ($operator_comment !== null ? $this->db->escape($operator_comment) : '') . "',
					 custom_field = '" . $this->db->escape($custom_field) . "' 
				 WHERE customer_id = " . (int)$customer_id
			);
	
			$customer_data['children'] = $children;
			$customer_data['operator_comment'] = $operator_comment;
			$customer_data['custom_field'] = $custom_field;
