    ...
    private function addOffers() {
        ...
        foreach ($products as $product) {
            ...
            foreach ($offers as $optionsString => $optionsValues) {
				...
				// Добавляем объем, если он есть
				if (!empty($product['volume'])) {
					$volume = $this->dd->createElement('param');
					$volume->setAttribute('code', 'volume'); // Уникальный код для объема
					$volume->setAttribute('name', 'Объем'); // Название поля для объема
					$volume->appendChild($this->dd->createTextNode($product['volume']));
					$catalog->appendChild($volume);
				}

				// Добавляем материал, если он есть
				if (!empty($product['material'])) {
					$material = $this->dd->createElement('param');
					$material->setAttribute('code', 'material'); // Уникальный код для материала
					$material->setAttribute('name', 'Материал'); // Название поля для материала
					$material->appendChild($this->dd->createTextNode($product['material']));
					$catalog->appendChild($material);
				}
            }
        }
    }
