...
class ModelCheckoutOrder extends Model {
	...
	$this->db->query("INSERT INTO `" . DB_PREFIX . "order` SET 
		...
		date_added = NOW(),
		date_modified = NOW(),
		desired_delivery_datetime = '" . $desired_delivery_datetime . "',
		need_call = '" . (int)$data['need_call'] . "'");



		$order_id = $this->db->getLastId();

		// Теперь логируем значения из базы
		$result = $this->db->query("SELECT desired_delivery_datetime, need_call FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$order_id . "'");
		$this->log->write("Data inserted for desired_delivery_datetime: " . $result->row['desired_delivery_datetime']);
		$this->log->write("Data inserted for need_call: " . $result->row['need_call']);
